...
import { AlertCircle, TrendingDown, ShoppingCart, Package } from 'lucide-react';
...
export function MaterialSourceSelector({
    ...
    const loadStockAvailability = async () => {
        ...
        try {
            ...
            const response = await api.post('/api/stock/availability', {
                        ...
                        >
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-4 text-sm">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded">
                                    ...
                                    <p className="font-semibold text-slate-900 dark:text-slate-100">{material.estimated_area.toFixed(2)} m²</p>
                                </div>
                            </div>
                                        ...
                                        {isLoadingStock ? (
                                            ...
                                            <div className="text-sm mt-2 space-y-1">
                                                <p className={`${
                                                    ...
                                                    {availability.stockCount} planche{availability.stockCount > 1 ? 's' : ''} disponible{availability.stockCount > 1 ? 's' : ''} - {availability.availableArea.toFixed(2)} m² total
                                                </p>
                                                {!hasStock && (
                                                ...
                                                <p className="text-slate-600 dark:text-slate-400 text-xs">
                                                    Coût estimé: {availability.estimatedCost.toFixed(2)} €
                                                </p>
                                            </div>
                                        ) : (
                                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                ...
                                <label className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                                            Coût estimé: {material.estimated_total_cost.toFixed(2)} €
                                        </p>
                                    </div>
                                </label>
                            </div>
