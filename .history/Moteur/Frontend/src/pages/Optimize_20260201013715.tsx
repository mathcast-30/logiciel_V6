...
import {
    Scissors, Play, AlertCircle, Download, Settings,
    ChevronLeft, ChevronRight, Sparkles, BarChart3, Package, AlertTriangle, Printer, Wrench,
    Rocket, CheckCircle2
} from 'lucide-react';
...
import { AIService, type BatchSuggestion, type GAParameters } from '../services/aiService';
...
export function Optimize() {
    ...
    const [aiSuggestions, setAiSuggestions] = useState<BatchSuggestion[]>([]);
    ...
    const [identifiedMaterials, setIdentifiedMaterials] = useState<any[]>([]);
    ...
    useEffect(() => {
        loadIdentifiedMaterials();
    }, [selectedPieceIds, selectedProjectIds]);

    const loadProjects = async () => {
        try {
            // Load projects first (critical)
            const pData = await ProjectService.getAll();
            setProjects(pData);
            if (pData.length > 0) setSelectedProjectIds([pData[0].id]);

            // Try to load AI suggestions (optional, non-blocking)
            try {
                const sData = await AIService.getBatchSuggestions();
                setAiSuggestions(sData);
            } catch (aiError) {
                console.warn('AI suggestions unavailable:', aiError);
                // AI failure is non-critical, continue
            }
        } catch (err) {
    ...
    // Load identified materials when pieces change (NEW)
    const loadIdentifiedMaterials = async () => {
        ...
        try {
            ...
            (response.data.materials || []).forEach((m: any) => {
    ...
    const runOptimization = async (forceUpdateStock = false) => {
        ...
        try {
            ...
            const response = await OptimizeService.run({
                ...
                stock_ids: selectedStockIds.length > 0 ? selectedStockIds : undefined,  // NEW
    ...
    const toggleProject = (id: number) => {
        setSelectedProjectIds(prev => {
            ...
            // Load history for newly selected projects
            if (!prev.includes(id) && newSelection.includes(id)) {
                loadOptimizationHistory(id);
            }

            return newSelection;
        });
        // Clear AI strategy if selection changes
        setAiStrategy(null);
    };

    const loadOptimizationHistory = async (projectId: number) => {
        try {
            const history = await OptimizeService.getHistory(projectId);
    ...
    const applyAiSuggestion = (suggestion: BatchSuggestion) => {
        setSelectedProjectIds(suggestion.project_ids);
        toast.info(`Suggestion "${suggestion.title}" appliquée !`);
    };
            ...
            {showSettings && (
                <div className="card p-6 animate-fade-in-down">
                    ...
                    <div className="border-t dark:border-slate-700/50 pt-6 mt-6 space-y-6">
                        ...
                        {currentMaterial && (
                            <StockSelector
                                materialId={parseInt(currentMaterial) || null}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 card p-6">
                            <h3 className="font-semibold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                                ...
                                Sélection des Projets (Mutualisation)
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto mb-6 pr-2 custom-scrollbar">
                                {projects.map(p => (
                                    <label
                                        key={p.id}
                                        className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all cursor-pointer ${selectedProjectIds.includes(p.id) ? 'border-indigo-500 bg-indigo-50/30 dark:bg-indigo-900/10' : 'border-slate-100 dark:border-slate-800 hover:border-slate-200'}`}
                                    >
                                        <input
                                            type="checkbox"
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium text-slate-700 dark:text-slate-200 truncate">{p.name}</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400">{p.parts?.length || 0} pièces</div>
                                        </div>
                                    </label>
                                ))}
                            </div>

                            <div className="flex items-center justify-between border-t dark:border-slate-700/50 pt-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm text-slate-700 dark:text-slate-300">Déduire du stock</span>
                                    </label>
                                </div>
                                <button
                                    onClick={() => runOptimization()}
                                    disabled={selectedProjectIds.length === 0 || isOptimizing}
                                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-xl flex items-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-indigo-200/50 dark:shadow-none"
                                >
                                    {isOptimizing ? (
                                        <><div className="spinner !w-5 !h-5 !border-white/30 !border-t-white" /> Calcul...</>
                                    ) : (
                                        <><Play className="h-5 w-5" /> Optimiser ({selectedProjectIds.length})</>
                                    )}
                                </button>
                            </div>
                        </div>

                        <div className="lg:col-span-1 card p-6 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/10 dark:to-purple-900/10 border-indigo-100 dark:border-indigo-900/30">
                            <h3 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                                <Sparkles className="h-5 w-5 text-indigo-500" />
                                Conseils de Batching IA
                            </h3>
                                ...
                                {aiSuggestions.length > 0 ? (
                                    aiSuggestions.map(s => (
                                        <button
                                            ...
                                            className="w-full text-left bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-transparent hover:border-indigo-500 transition-all group"
                                        >
                                            <div className="flex items-start justify-between mb-2">
                                                <div className="font-bold text-slate-800 dark:text-white group-hover:text-indigo-600 text-sm">{s.title}</div>
                                                <div className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full">{s.potential_saving}</div>
                                            </div>
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-2">{s.description}</p>
                                            <div className="flex flex-wrap gap-1">
                                                {s.project_names.map(name => (
                                                    <span key={name} className="text-[9px] bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded italic">{name}</span>
                                                ))}
                                            </div>
                                        </button>
                                    ))
                                    ...
                                    <div className="text-center py-6">
                                        ...
                                        <p className="text-xs text-slate-500 px-4">Utilisez plus de projets pour obtenir des conseils de regroupement.</p>
                                    </div>
                                )}
                            </div>
                        </div>
