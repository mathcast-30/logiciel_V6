...
export function Optimize() {
    ...
    useEffect(() => {
        ...
        const loadMaterials = async () => {
            try {
                const response = await api.post('/api/materials/identify-from-pieces', {
                ...
                (response.data || []).forEach((m: { id: number }) => {
    ...
    const runOptimization = async (forceUpdateStock = false) => {
        ...
        try {
            ...
            const response = await OptimizeService.run({
                ...
                material_sources: Object.keys(materialSources).length > 0 ? materialSources : undefined
            });
            setResult(response);
