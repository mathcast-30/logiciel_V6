...
wsl sudo -u postgres psql -d opticut_pro -c "CREATE SCHEMA IF NOT EXISTS opticum;" >nul 2>&1
wsl sudo -u postgres psql -d opticut_pro -c "CREATE TABLE IF NOT EXISTS opticum.metadata (id SERIAL PRIMARY KEY, key VARCHAR(255) UNIQUE, value TEXT, created_at TIMESTAMP DEFAULT NOW());" >nul 2>&1
wsl sudo -u postgres psql -d opticut_pro -c "INSERT INTO opticum.metadata (key, value) VALUES ('version', '4.0.0'), ('storage_type', 'PostgreSQL Advanced'), ('wal_enabled', 'true');" >nul 2>&1
