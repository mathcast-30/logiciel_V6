...
export const MaterialBreakdown = memo(function MaterialBreakdown({
    materials,
    isLoading = false
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {materials.map((material) => (
                            <tr key={material.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${material.is_panel
                                ...
                                <td className="text-right px-4 py-3 text-slate-700 dark:text-slate-300">
                                    <span className="font-medium">{material.estimated_area?.toFixed(2) ?? "0.00"}</span>
                                    <span className="text-xs text-slate-600 dark:text-slate-400 ml-1">m²</span>
                                </td>
                                <td className="text-right px-4 py-3">
                                    <span className="font-medium text-slate-900 dark:text-slate-100">
                                        {material.estimated_total_cost?.toFixed(2) ?? "0.00"} €
                                    </span>
                                </td>
                            </tr>
                        ))}
