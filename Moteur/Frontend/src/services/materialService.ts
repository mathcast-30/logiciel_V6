...
export interface MaterialAvailability {
    ...
    material_species?: string;
    is_panel: boolean;
    thickness: number;
    stock_count: number;
    available_area: number;
    available_panels: StockPanel[];
...
export const MaterialService = {
    ...
    checkAvailability: async (materialIds: number[]): Promise<AvailabilityResponse> => {
        const response = await api.post<AvailabilityResponse>('stock/availability', { material_ids: materialIds });
        return response.data;
    },
};
